...
//			applyFilter med = new applyFilter();
//			med.af();
			Collections.sort(pix);	//ohne sortieren vorher wird das Bild nur verschoben nach links unten
			int pixMed = pix.get((pix.size()/2)+(pix.size()/2-1)/2); //formel für median
			int pos = y * widthImage + x;
			destinationImage.argb[pos] = (0xff000000) | (pixMed << 16) | (pixMed << 8) | pixMed;
			}
		}
	}
}
